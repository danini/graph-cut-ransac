...
template<class GravityEssentialMatrixEstimator>
	...
	int lo_number)
{
	...
	GravityEssentialMatrixEstimator estimator(intrinsics_src,
		intrinsics_dst);
	...
	inlier_selector::EmptyInlierSelector<GravityEssentialMatrixEstimator, 
		AbstractNeighborhood> inlier_selector(neighborhood_graph.get());

	if (use_sprt)
	{
		// Initializing SPRT test
		preemption::SPRTPreemptiveVerfication<GravityEssentialMatrixEstimator> preemptive_verification(
			points,
			estimator,
			min_inlier_ratio_for_sprt);

		GCRANSAC<GravityEssentialMatrixEstimator,
			AbstractNeighborhood,
			MSACScoringFunction<GravityEssentialMatrixEstimator>,
			preemption::SPRTPreemptiveVerfication<GravityEssentialMatrixEstimator>> gcransac;
	...
	else
	{
		...
		GCRANSAC<GravityEssentialMatrixEstimator, AbstractNeighborhood> gcransac;
		gcransac.settings.threshold = threshold / threshold_normalizer; // The inlier-outlier threshold
		gcransac.settings.spatial_coherence_weight = spatial_coherence_weight; // The weight of the spatial coherence term
		gcransac.settings.confidence = conf; // The required confidence in the results
		gcransac.settings.max_local_optimization_number = lo_number; // The maximum number of local optimizations
	...
	if (statistics.inliers.size() > 5)
	{
		...
		// Scoring function
		MSACScoringFunction<GravityEssentialMatrixEstimator> scoring;
