...
template<class FundamentalMatrixEstimator>
int findFundamentalMatrix_(
	...
	int lo_number, 
    // Initialize the bundle adjustment optimizer with the model
    bool initialize_BA)
{
	...
	inlier_selector::EmptyInlierSelector<FundamentalMatrixEstimator, 
		AbstractNeighborhood> inlier_selector(neighborhood_graph.get());

	if (use_sprt)
	{
		// Initializing SPRT test
		preemption::SPRTPreemptiveVerfication<FundamentalMatrixEstimator> preemptive_verification(
			points,
			estimator,
			min_inlier_ratio_for_sprt);

		GCRANSAC<FundamentalMatrixEstimator,
			AbstractNeighborhood,
			MSACScoringFunction<FundamentalMatrixEstimator>,
			preemption::SPRTPreemptiveVerfication<FundamentalMatrixEstimator>> gcransac;
	...
	{
		GCRANSAC<FundamentalMatrixEstimator, AbstractNeighborhood> gcransac;
		gcransac.settings.threshold = threshold; // The inlier-outlier threshold
		gcransac.settings.spatial_coherence_weight = spatial_coherence_weight; // The weight of the spatial coherence term
		gcransac.settings.confidence = conf; // The required confidence in the results
		gcransac.settings.max_local_optimization_number = lo_number; // The maximum number of local optimizations
	...
	if (statistics.inliers.size() > 7)
	{
		...
		// Scoring function
		MSACScoringFunction<FundamentalMatrixEstimator> scoring;
