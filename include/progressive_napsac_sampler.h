...
class ProgressiveNapsacSampler : public theia::Sampler < cv::Mat, size_t >
{
protected:
	...
	std::vector<GridNeighborhoodGraph> grid_layers; // The overlapping neighborhood grids
	...
	theia::ProsacSampler one_point_prosac_sampler, // The PROSAC sampler used for selecting the initial points, i.e., the center of the hypersphere.
...
bool ProgressiveNapsacSampler::initialize(const cv::Mat const *container_)
{
	...
	for (size_t layer_idx = 0; layer_idx < layer_number; ++layer_idx)
	{
		...
		grid_layers.emplace_back(GridNeighborhoodGraph(container_,
...
inline bool ProgressiveNapsacSampler::sample(
